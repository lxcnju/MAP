...
class FedAPObser():
    ...
    def train(self):
        ...
        for r in range(1, self.args.max_round + 1):
            ...
            for client in sam_clients:
                ...
                # update local model with RS
                local_model, loss, local_infos1 = self.update_local(
                    r=r,
                    client=client,
                ...
                if client == sam_clients[0]:
                    self.logs["LOCAL_INFOS"].extend(local_infos1)
                    self.logs["LOCAL_INFOS"].extend(local_infos2)
    ...
    def update_local(
            self, r, client, model, train_loader, test_loader, dist):
        lr = self.args.lr
        ...
        # local training with Restricted Softmax
        model.train()
    ...
    def update_local_hpm(
        ...
        n_total_bs = max(int(n_total_bs * 0.1), 5)
        ...
        for t in range(n_total_bs + 1):
            if t in [0, n_total_bs]:
                per_acc, per_mf1, infos = self.local_test(
                    model=model,
                    hpm=hpm,
                    loader=test_loader,
                )
        ...
        return hpm, per_accs, per_mf1s, local_infos
    ...
    def local_test(self, model, hpm, loader):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(loader):
                ...
                reals.append(batch_y.cpu().detach().numpy())
                feats.append(phs.cpu().detach().numpy())
