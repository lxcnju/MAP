...
class FedDML():
    ...
    def update_local(self, r, model, train_loader, test_loader):
        ...
        for t in range(n_total_bs + 1):
            ...
            ce_loss1 = criterion(logits, batch_y)
            ce_loss2 = criterion(plogits, batch_y)

            # knowledge transfer
            kt_loss_1 = self.knowledge_transfer_loss(logits, plogits)
            kt_loss_2 = self.knowledge_transfer_loss(plogits, logits)

            c1 = self.args.kt_alpha
            c2 = self.args.kt_beta
            loss1 = c1 * ce_loss1 + (1.0 - c1) * kt_loss_1
            loss2 = c2 * ce_loss2 + (1.0 - c2) * kt_loss_2
    ...
    def knowledge_transfer_loss(self, logits1, logits2):
        kl_loss = F.kl_div(
            F.log_softmax(logits1, dim=-1),
            F.softmax(logits2.detach() / self.args.kt_tau, dim=-1),
            reduction="mean"
        )
        return kl_loss
