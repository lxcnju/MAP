...
def main(algo):
    ...
    for d in [0, 2]:
        dataset = datasets[d]
        net = nets[d]
        local_epochs = 1    # only for demo
        n_clients = 100
        c_ratio = 0.02      # only for demo
        max_round = 2       # only for demo
        lr = lrs[d]

        for j in range(hypers["cnt"])[0:1]:
            para_dict = {}
            ...
            para_dict["fname"] = "demo-maxround={}-localepoch={}-cratio={}.log".format(
                max_round, local_epochs, c_ratio
            )
